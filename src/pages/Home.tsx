...
function Home() {
  ...
  const { notes, setSelectedNote, setSelectedFolder, archived, deleted, folders } = useAppContext();
  ...
  const getFilteredNotes = () => {
    ...
    if (view === 'favorites') {
      filteredNotes = notes.filter(note => note.isFavorite && !note.isDeleted);
    } else if (view === 'trash') {
      filteredNotes = deleted.filter(note => note.deletedAt != null);
    } else if (view === 'archived') {
      filteredNotes = archived.filter(note => note.isArchived && note.deletedAt === null)
    } else if (folderId) {
      filteredNotes = notes.filter(note => note.folderId === folderId);
    } else {
      filteredNotes = notes.filter(note => note.folderId === folders[0].id && !note.isArchived && !note.isDeleted);
