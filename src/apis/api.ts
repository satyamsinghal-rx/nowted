...
export const getArchived = async (): Promise<Note[]> => {
    try {
        const response = await axios.get<{ notes: Note[] }>(`${API_URL}/notes?archived=true`);
        return response.data.notes;
    } catch (error) {
        console.log(error);
        return [];
...
export const getRecents = async (): Promise<Note[]> => {
    try {
        const response = await axios.get<{ recentNotes: Note[] }>(`${API_URL}/notes/recent`);
...
export const getFolderById = async (id: string): Promise<Folder | null> => {
    try {
        const response = await axios.get<{ folder: Folder }>(`${API_URL}/folders/${id}`);
...
export const createFolder = async (folder: Partial<Folder>): Promise<Folder | null> => {
    try {
        const response = await axios.post<{ folder: Folder }>(`${API_URL}/folders`, folder)
        return response.data.folder;
    } catch (error) {
        console.log(error);
        return null;
...
export const createNote = async (note: Partial<Note>): Promise<Note | undefined> => {
    try {
...
export const editFolder = async (id: string, folder: Partial<Folder>): Promise<Folder | null> => {
    try {
        const response = await axios.patch<{ folder: Folder }>(`${API_URL}/folders/${id}`, folder);
        return response.data.folder;
    } catch (error) {
        console.log(error);
        return null;
